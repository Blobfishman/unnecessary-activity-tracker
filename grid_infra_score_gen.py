...
def main(args):
    ...
    if not (len(args) == 4 or len(args) == 7):
        ...
        print("python3 grid_infra_score_gen.py GRID.shp TAG_WEIGHTS.csv "
              "INFRA_SCORES.shp (BLUR_RADIUS GRID_WIDTH GRID_HEIGHT)")
        print("(BLUR RADIUS GRID_WIDTH GRID_HEIGHT) is optional, if you want "
              "to blur the outcoming infrastructure values over the grid.")
        return
    if len(args) == 7:
        if isinstance(args[5], (int, float)):
            blur_radius = float(args[4])
        else:
            print("Invalid blur radius. Input a float! Usage: ")
            print("python3 grid_infra_score_gen.py GRID.shp TAG_WEIGHTS.csv"
                  "INFRA_SCORES.shp (BLUR_RADIUS GRID_WIDTH GRID_HEIGHT)")
            print("(BLUR RADIUS GRID_WIDTH GRID_HEIGHT) is optional, if you "
                  "want to blur the outcoming infrastructure values over the "
                  "grid.")
            return
        if isinstance(args[5], int) and isinstance(args[6], int):
            grid_width = int(args[5])
            grid_height = int(args[6])
        else:
            print("Invalid grid dimensions GRID_WIDTH and GRID_HEIGHT. Input"
                  " Integers! Usage:")
            print("python3 grid_infra_score_gen.py GRID.shp TAG_WEIGHTS.csv "
                  "INFRA_SCORES.shp (BLUR_RADIUS GRID_WIDTH GRID_HEIGHT)")
            print("(BLUR RADIUS GRID_WIDTH GRID_HEIGHT) is optional, if you"
    ...
    try:
        ...
        rnd = 4
        bb = str(round(miny, rnd)) + "," \
            + str(round(minx, rnd)) + "," \
            + str(round(maxy, rnd)) + "," \
            + str(round(maxx, rnd))
        print("Found boundary box (", bb,  ")")
    except:
        print("Grid shape file could not be located or is in the wrong format."
              " Do you also have the .shx and .dbf files in the same "
              "directory? Aborting..")
        return
    if len(args) == 7:
        if not grid_width*grid_height == len(grid['geometry']):
            print("Given grid dimensons do not match given grid file.")
            print(str(grid_width)+"*"+str(grid_height) + "="
                  + str(grid_width*grid_height)+"≠"+str(len(grid["geometry"])))
            print("Aborting..")
            return
    ...
    for i in range(0, len(df.values)-58):
        for x in range(2, n_cols+2):
            ...
            if not string == "nan":
                ...
                for value in liststring:
                    ...
                    print(df.columns[x] + ":" + value + " ["
                          + str(df.values[i][n_cols+2]) + "]")
                    for pre in ["node", "way"]:
                        resultstring += "\t" + pre + '[\"' + df.columns[x] \
                            + '\"=\"' + value + '\"](' + bb + ');'
    ...
    try:
        ...
        print("Query failed. Either there is no internet connection or the "
              "query timed out. Aborting..")
        return

    print("Finished query!")
    print(str(len(result.nodes)) + " nodes found."
          "(also containing the unzipped ways)")
    ...
    # Target values: id, infra_wert, geometry
    ...
    #     ct+=1
    # except:
    #     print("WARNING: Invalid way-shape fount @id=", w.id, "i=", i,
    #           " ignoring...")
    #     pass
    ...
    # polypivot = pd.pivot_table(polysjoin, index="id",
    #                            aggfunc={'infra_score': np.sum})
    # # dfpivot.columns = dfpivot.columns.droplevel()
    #
    ...
    if len(args) == 7:
        ...
        # da nicht alle grids quadratisch sind,
        # aber in unserem Fall funtkioniert sie
        # width = int(np.floor(np.sqrt(N)))
        # height= int( np.ceil(np.sqrt(N)))

        # Mögliche Lösungen:
        # I  : Der user gibt width und height an
        # II : Es gibt einen intelligenteren Weg width und height zu bestimmen
